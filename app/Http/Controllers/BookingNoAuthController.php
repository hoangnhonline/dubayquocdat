    ...
    */
    public function index(Request $request)
    {
       ...
       // dd($items);
        ...
        $beachList = Beach::orderBy('display_order')->get();
        $beachArr = [];
        foreach($beachList as $beach){
            $beachArr[$beach->id] = $beach->name;
        }
        $partners = Account::where('is_partner', 1)->get();
        return view($view, compact( 'items', 'arrSearch', 'keyword', 'time_type', 'month', 'year', 'month_do', 'arrData', 'chi_tien_mat', 'chietkhauList', 'chi_khac', 'beach_id', 'cateList', 'cate_id', 'use_date_from_format', 'use_date_to_format', 'beachList', 'beachArr', 'partners', 'partner_id'));
